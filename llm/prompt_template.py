...
# 2 病机 {"唾痰": ["A", 0]}
# template_prompt2 = """您是一位中医学专家，请针对每条临床表现信息，结合病情描述，从病机候选项中，选择最相关的一条或多条记录标签（A~J），并根据相关性和重要性打分，分值范围1、2或者3，分值越高重要性越高，不相关的不要显示，严格按照json格式输出。
template_prompt2 = """您是一位中医学专家，请针对每条临床表现信息，结合病情描述，从病机候选项中，选择最相关的一条记录标签（A~J），并根据相关性和重要性打分，分值范围1、2或者3，分值越高重要性越高，不相关的不要显示，严格按照json格式输出。
注意不要Explanation。
...
# 3 证候 {"肾水不足": ["B", 0]}
template_prompt3 = """您是一位中医学专家，请针对每条病机信息，结合病情描述，从证候候选项中，选择相关的一条或多条记录标签（A~J），并根据相关性和重要性打分，分值范围1、2或者3，分值越高重要性越高，不相关的不要显示，严格按照json格式输出。
注意不要Explanation。
