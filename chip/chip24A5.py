...
from utils import dict_utils, ee_wrapper, file_utils, json_utils, local_wrapper
...
# 1 临床表现信息（实体）：，不包括体温等指标型数据
template_prompt1 = """您是一位中医学专家，请根据下面这段文字提取主要的临床表现信息（包括症状描述），严格按照json格式返回。
注意严格保留原文。
病情描述：{raw_text}
...
def predict_2(raw_text, result1, candidate2):
    ...
    response2_local = local_wrapper.chat_complete(prompt2)
    print(response2_local)
    response2_local_obj = json_utils.cvt_str_to_obj(response2_local)
    response2_local_obj2 = {}
    for k1 in response2_local_obj:
        k2 = response2_local_obj[k1][0]
        if k2 not in candidate2_map and k2 in candidate2_map_r:
            k2 = candidate2_map_r[k2]
        response2_local_obj2[k1] = [k2, response2_local_obj[k1][1]]

    response2_obj = response2_local_obj2
...
def predict_3(raw_text, result2_full, candidate3):
    ...
    response3_local = local_wrapper.chat_complete(prompt3)
    print(response3_local)
    response3_local_obj = json_utils.cvt_str_to_obj(response3_local)
    response3_local_obj2 = {}
    for k1 in response3_local_obj:
        k2 = response3_local_obj[k1][0]
        if k2 not in candidate3_map and k2 in candidate3_map_r:
            k2 = candidate3_map_r[k2]
        response3_local_obj2[k1] = [k2, response3_local_obj[k1][1]]

    response3_obj = response3_local_obj2
